...
func TestImportMovesSupportedMediaAndIndexesIt(t *testing.T) {
	for name, organize := range map[string]bool{"preserve": false, "date": true} {
		t.Run(name, func(t *testing.T) {
			...
			job, err := service.StartWithOptions(ctx, principal, "legacy", admin.ID, organize)
			if err != nil {
				t.Fatal(err)
			}
			service.Wait()
			...
			if organize {
				destinationRelative = filepath.ToSlash(filepath.Join("users", admin.ID, "Imported", "2020", "05", "06", "found.jpg"))
			}
...
func TestImportSkipsBothFilesWhenMP4PairedStillDestinationExists(t *testing.T) {
	store, err := storage.New(filepath.Join(t.TempDir(), "photos"))
	if err != nil {
		t.Fatal(err)
	}
	...
	file, err := os.Create(filepath.Join(sourceDir, "IMG_42.JPG"))
	if err != nil {
		t.Fatal(err)
	}
	if err := jpeg.Encode(file, image.NewRGBA(image.Rect(0, 0, 2, 2)), nil); err != nil {
	...
	if err := os.WriteFile(filepath.Join(sourceDir, "img_42.MP4"), heifBytesForImporter("isom"), 0o640); err != nil {
		t.Fatal(err)
	}
	targetStill, err := store.ResolvePath(filepath.Join("users", "user-import", "Imported", "camera", "IMG_42.JPG"))
	if err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(filepath.Dir(targetStill), 0o750); err != nil {
	...
	motionSource := filepath.Join(sourceDir, "img_42.MP4")
	if _, statErr := os.Stat(motionSource); statErr != nil {
		t.Fatalf("paired MP4 stat error = %v, want source companion preserved", statErr)
	}
	motionTarget, err := store.ResolvePath(filepath.Join("users", "user-import", "Imported", "camera", "img_42.MP4"))
	if err != nil {
		t.Fatal(err)
	}
	if _, statErr := os.Stat(motionTarget); !os.IsNotExist(statErr) {
