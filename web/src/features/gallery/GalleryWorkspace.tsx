...
function PhotoTile({ photo, onSelect }: { photo: Photo; onSelect: (photo: Photo) => void }) {
  ...
  return <figure className="photo-tile" onClick={() => onSelect(photo)}><div className={`photo-frame ${failed ? 'is-failed' : ''}`}>{failed ? <span>{t('gallery.previewPending')}</span> : <img src={`/api/v1/photos/${encodeURIComponent(photo.id)}/thumbnail?size=256`} alt={photo.filename} loading="lazy" onError={() => setFailed(true)} />}{photo.is_live_photo && !failed && <span title="Live Photo" style={{ position: 'absolute', left: 9, top: 9, display: 'inline-flex', alignItems: 'center', gap: 4, borderRadius: 999, padding: '5px 7px', background: 'rgba(32,37,45,.72)', color: '#fff', fontSize: 9, fontWeight: 700, letterSpacing: '.08em', pointerEvents: 'none' }}><CirclePlay size={12} /> LIVE</span>}</div><figcaption>{photo.filename}</figcaption></figure>;
}
