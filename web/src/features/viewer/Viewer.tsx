...
import { ArrowLeft, ArrowRight, Download, Share2, Trash2, X } from 'lucide-react';
...
export default function Viewer({ api, photos, selected, onClose, onDeleted, onUpdated }: { api: ApiClient; photos: Photo[]; selected: number; onClose: () => void; onDeleted: (id: string) => void; onUpdated: (photo: Photo) => void }) {
  ...
  async function renamePhoto() { if (!name.trim() || name.trim() === photo.filename) return; setBusy(true); try { const updated = await api.renamePhoto(photo.id, name.trim()); onUpdated(updated); setName(updated.filename); setMessage(t('viewer.nameUpdated')); } catch { setMessage(t('viewer.renameFailed')); } finally { setBusy(false); } }
  async function movePhoto() { if (!moveFolder || moveFolder === photo.folder_id) return; setBusy(true); try { const updated = await api.movePhoto(photo.id, moveFolder); onUpdated(updated); void api.getFolder(updated.folder_id).then(setCurrentFolder).catch(() => setCurrentFolder(null)); setMessage(t('viewer.moved')); } catch { setMessage(t('viewer.moveFailed')); } finally { setBusy(false); } }
