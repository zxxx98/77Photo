...
4. 在 App 中添加服务器地址。HTTPS 域名、HTTPS IP 和带端口地址都可以使用；HTTP 只能使用命中的内网 IP CIDR，并会在登录页显示未加密警告。
5. 使用用户名和密码完成移动登录。密码不会转化为 Web Cookie；后续请求使用短期 access token，refresh token 轮换并只保存在 Android Keystore 加密存储中。

HTTP 内网白名单是风险开关，不是加密替代品。默认范围包括 loopback、RFC1918、link-local 和 ULA；用户添加公网 CIDR 前会收到高风险确认，`0.0.0.0/0` 和 `::/0` 永远拒绝。不要把公网反向代理地址配置成 HTTP。
