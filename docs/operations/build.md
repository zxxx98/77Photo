...
SQLite 使用 `modernc.org/sqlite`，是纯 Go driver；当前服务和 SQLite 路径不要求 CGO 或系统动态图片库。容器构建显式使用 `CGO_ENABLED=0`，CI 对 `linux/amd64` 与 `linux/arm64` 分别执行 BuildKit 构建。

如果后续引入 libvips 或其他本地图片库，必须同时更新 Docker 构建阶段、运行时动态库清单和两个架构的 CI 冒烟验证，不能把宿主机上的库默认为发布依赖。
